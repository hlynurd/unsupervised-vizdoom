...
def create_network(session, available_actions_count):
    ...
    loss = tf.losses.mean_squared_error(q, target_q_)    #X

    optimizer = tf.train.RMSPropOptimizer(learning_rate) #X
    # Update the parameters according to the computed gradient using RMSProp.
    train_step = optimizer.minimize(loss)                #X
...
def learn_from_memory():
    ...
    if memory.size > batch_size:
        ...
        q2 = np.max(keras_controller.predict(s2))#(get_q_values(s2), axis=1)
        target_q = keras_controller.predict(s1)#(s1)
        # target differs from q only for the selected action. The following means:
        # target_Q(s,a) = r + gamma * max Q(s2,_) if isterminal else r
        target_q[np.arange(target_q.shape[0]), a] = r + discount_factor * (1 - isterminal) * q2
        learn(s1, target_q)
        keras_controller.fit(s1, target_q, epochs=1, batch_size=batch_size, verbose = 0)
...
if __name__ == '__main__':
    ...
    keras_controller = sfa_vizdoom_default_cnn(resolution[0], resolution[1], len(actions))
